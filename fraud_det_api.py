...
# Load model
trained_mod = joblib.load("fraud_det_model.pkl")
...
# Function predicts whether transaction are fraudulent or not
def predict(transaction: Transaction):
    
    # Input transaction data
    input_data = np.array([[transaction.Time, transaction.V1, transaction.V2, transaction.V3, transaction.V4,
    ...
    pred = trained_mod.predict(input_data)
    fraud_prob = trained_mod.predict_proba(input_data)[0][1]
    
    # Return prediction result
    return
    {
        "fraud prediction": bool(pred[0]),
